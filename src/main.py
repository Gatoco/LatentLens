...
def _get_popular_movies(top_n: int = 10) -> List[Dict]:
    ...
    try:
        ...
        movie_stats = data_loader.ratings.groupby('movieId').agg({
            'rating': ['count', 'mean']
        }).round(2)
        ...
        for _, movie in popular_movies.head(top_n).iterrows():
            movie_info = data_loader.movies[data_loader.movies['movieId'] == movie['movieId']]
...
def _get_trending_movies(top_n: int = 10) -> List[Dict]:
    ...
    try:
        ...
        data_loader.movies['year'] = data_loader.movies['title'].str.extract(r'\((\d{4})\)$')[0]
        data_loader.movies['year'] = pd.to_numeric(data_loader.movies['year'], errors='coerce')
        
        # Filter recent movies
        recent_threshold = data_loader.movies['year'].max() - 10  # Last 10 years
        recent_movies = data_loader.movies[data_loader.movies['year'] >= recent_threshold]
        
        # Get rating stats for recent movies
        recent_movie_ids = recent_movies['movieId'].tolist()
        recent_ratings = data_loader.ratings[data_loader.ratings['movieId'].isin(recent_movie_ids)]
        
        movie_stats = recent_ratings.groupby('movieId').agg({
            'rating': ['count', 'mean']
        }).round(2)
        ...
        trending_movies = movie_stats[
            (movie_stats['rating_count'] >= 20) & 
            (movie_stats['avg_rating'] >= 3.8)
        ]
        
        # Sort by average rating, then by count
        trending_movies = trending_movies.sort_values(['avg_rating', 'rating_count'], ascending=[False, False])
...
def _get_diverse_genre_recommendations(top_n: int = 10) -> List[Dict]:
    ...
    try:
        ...
        all_genres = []
        for genres_str in data_loader.movies['genres'].dropna():
            genres = genres_str.split('|')
            all_genres.extend(genres)
        
        from collections import Counter
        genre_counts = Counter(all_genres)
        top_genres = [genre for genre, _ in genre_counts.most_common(8) if genre != "(no genres listed)"]
        ...
        for genre in top_genres:
            ...
            genre_movies = data_loader.movies[data_loader.movies['genres'].str.contains(genre, na=False)]
            genre_movie_ids = genre_movies['movieId'].tolist()
            
            # Get rating stats for genre movies
            genre_ratings = data_loader.ratings[data_loader.ratings['movieId'].isin(genre_movie_ids)]
            ...
            if len(genre_ratings) > 0:
                ...
                good_movies = good_movies.sort_values('avg_rating', ascending=False)
...
async def get_hybrid_recommendations(
    ...
    try:
        ...
        if is_new_user:
            ...
            diverse_movies = _get_diverse_genre_recommendations(top_n=max(3, top_n//3))
            ...
            for movie in popular_movies:
                if movie['movieId'] not in seen_movie_ids and len(cold_start_recommendations) < top_n:
            ...
            return {
                ...
                "strategy_breakdown": {
                    "popular_movies": len([r for r in cold_start_recommendations if r['reason'] == 'popular_movie']),
                    "trending_movies": len([r for r in cold_start_recommendations if r['reason'] == 'trending_movie']),
                    "diverse_genre_movies": len([r for r in cold_start_recommendations if 'top_in_genre' in r['reason']])
                }
            }
        
        elif not has_sufficient_data:
            logger.info(f"Cold start: User {user_id} has insufficient ratings for full collaborative filtering")
            ...
            user_movie_info = data_loader.movies[data_loader.movies['movieId'].isin(user_movies)]
            user_genres = set()
            for genres_str in user_movie_info['genres'].dropna():
            ...
            if user_genres:
                for genre in list(user_genres)[:3]:  # Top 3 user genres
                    ...
                    genre_movies_filtered = [m for m in genre_movies if genre.lower() in m['reason']]
            ...
            for movie in genre_based_recommendations:
                if movie['movieId'] not in seen_movie_ids and len(limited_data_recommendations) < top_n:
                    seen_movie_ids.add(movie['movieId'])
...
async def get_cold_start_recommendations(
    ...
    try:
        ...
        logger.error(f"Error generating cold start recommendations for user {user_id}: {e}")
...
async def get_new_movies(
    ...
    try:
        ...
        if min_year is None:
            min_year = data_loader.movies['year'].max() - 5
        
        # Filter recent movies
        recent_movies = data_loader.movies[
            (data_loader.movies['year'] >= min_year) & 
            (data_loader.movies['year'].notna())
        ]
        ...
        for _, movie in recent_movies.iterrows():
            ...
            movie_info = {
                ...
                "cold_start_level": "high" if len(movie_ratings) < 10 else "medium" if len(movie_ratings) < 50 else "low"
            }
        ...
        return {
            ...
            "movies": result[:limit],
            "cold_start_summary": {
                "extreme": len([m for m in result if m['cold_start_level'] == 'extreme']),
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving new movies: {str(e)}")

@app.get("/recommend/for-new-movie/{movie_id}", tags=["Cold Start"])
async def get_recommendations_for_new_movie(
    movie_id: int,
    top_n: int = 10
):
    """
    ...
    try:
        ...
        target_movie = data_loader.movies[data_loader.movies['movieId'] == movie_id]
        if target_movie.empty:
            raise HTTPException(status_code=404, detail=f"Movie {movie_id} not found")
        
        target_movie = target_movie.iloc[0]
        target_genres = set(target_movie['genres'].split('|')) if pd.notna(target_movie['genres']) else set()
        
        # Get rating count for target movie
        target_ratings = data_loader.ratings[data_loader.ratings['movieId'] == movie_id]
        target_rating_count = len(target_ratings)
        target_avg_rating = target_ratings['rating'].mean() if len(target_ratings) > 0 else None
        
        # Find similar movies based on genres
        similar_movies = []
        
        for _, movie in data_loader.movies.iterrows():
            ...
            if genre_similarity > 0:
                ...
                movie_ratings = data_loader.ratings[data_loader.ratings['movieId'] == movie['movieId']]
                ...
                similar_movies.append({
                    ...
                    "rating_count": len(movie_ratings),
                    "avg_rating": float(movie_ratings['rating'].mean()) if len(movie_ratings) > 0 else None
                })
        
        # Sort by genre similarity and rating quality
        similar_movies.sort(key=lambda x: (x['genre_similarity'], x['avg_rating'] or 0), reverse=True)
        ...
        return {
            "target_movie": {
                ...
                "avg_rating": float(target_avg_rating) if target_avg_rating is not None else None,
                "cold_start_level": "high" if target_rating_count < 10 else "medium" if target_rating_count < 50 else "low"
            },
