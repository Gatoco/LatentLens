...
class TestColdStartDetection:
    ...
    def test_insufficient_data_detection(self, data_loader):
        ...
        if len(users_with_few_ratings) > 0:
            ...
            assert len(user_ratings) < 5, "User should have insufficient data for reliable recommendations"
    ...
    def test_cold_start_threshold_detection(self, data_loader):
        ...
        for threshold in thresholds:
            ...
            for user_id in cold_start_users.index[:5]:  # Test first 5 users
                ...
                assert user_count < threshold, f"User {user_id} should be below threshold {threshold}"
...
class TestColdStartStrategies:
    ...
    def test_popular_strategy_cold_start(self, mock_recommender, client):
        ...
        for movie in json_response["recommendations"]:
            assert movie["average_rating"] > 4.0, "Popular movies should have high ratings"
            assert movie["num_ratings"] > 100, "Popular movies should have many ratings"
        
        mock_recommender.recommend.assert_called_once_with(999999, "cold_start", 2, strategy="popular")
    ...
    def test_trending_strategy_cold_start(self, mock_recommender, client):
        ...
        for movie in json_response["recommendations"]:
            ...
            assert movie["recent_popularity"] == True, "Movies should be marked as trending"
        
        mock_recommender.recommend.assert_called_once_with(888888, "cold_start", 2, strategy="trending")
    ...
    def test_diverse_strategy_cold_start(self, mock_recommender, client):
        ...
        assert len(unique_genres) == 3, "Diverse strategy should provide different genres"
        
        mock_recommender.recommend.assert_called_once_with(777777, "cold_start", 3, strategy="diverse")
...
class TestColdStartAPIIntegration:
    ...
    def test_cold_start_error_handling(self, client):
        ...
        with patch('src.main.recommender') as mock_recommender:
            mock_recommender.recommend.side_effect = Exception("Cold start service error")
...
class TestColdStartDataGeneration:
    ...
    def test_popular_movies_generation(self, data_loader):
        ...
        popular_movies = movie_stats[
            ...
            (movie_stats['average_rating'] >= 4.0)
        ].sort_values('average_rating', ascending=False)
        
        # Verify we have popular movies for cold start
        assert len(popular_movies) > 0, "Should have popular movies available for cold start"
        ...
        for movie_id, stats in popular_movies.head(10).iterrows():
            assert stats['num_ratings'] >= 100, f"Movie {movie_id} should have ≥100 ratings"
            assert stats['average_rating'] >= 4.0, f"Movie {movie_id} should have ≥4.0 rating"
    ...
    def test_trending_movies_generation(self, data_loader):
        ...
        assert len(recent_movies) > 0, "Should have recent movies for trending recommendations"
        assert len(recent_movies) >= 100, "Should have substantial number of recent movies"
        
        # Verify year extraction works
        for _, movie in recent_movies.head(10).iterrows():
            assert movie['year'] >= 2014, f"Movie should be from 2014 or later: {movie['title']}"
    ...
    def test_diverse_genre_generation(self, data_loader):
        ...
        assert len(all_genres) >= 15, f"Should have at least 15 genres, found {len(all_genres)}"
        
        # Verify common genres are present
        expected_genres = ["Action", "Comedy", "Drama", "Thriller", "Romance", "Adventure"]
...
class TestColdStartPerformance:
    ...
    def test_cold_start_response_time(self, mock_recommender, client):
        ...
        assert response.status_code == 200
        assert response_time < 5.0, f"Cold start response should be <5s, got {response_time:.2f}s"
    ...
    def test_cold_start_recommendation_quality(self, mock_recommender, client):
        ...
        assert recommendation["average_rating"] > 4.0, "Cold start recommendations should be high quality"
        assert recommendation["num_ratings"] > 100, "Cold start recommendations should be well-established"
