...
class TestColdStartStrategies:
    ...
    def test_valid_cold_start_strategies(self, client):
        ...
        for strategy in valid_strategies:
            response = client.get(f"/recommend/cold-start/{non_existent_user}?strategy={strategy}&limit=3")
            ...
            assert response.status_code in [200, 500], f"Strategy '{strategy}' should be handled"
    ...
    def test_invalid_cold_start_strategy_handling(self, client):
        ...
        for strategy in invalid_strategies:
            ...
            assert response.status_code in [200, 422, 500], f"Invalid strategy '{strategy}' should be handled"
...
class TestColdStartPerformance:
    ...
    def test_cold_start_response_time(self, client):
        ...
        assert response_time < 10.0, f"Cold start response too slow: {response_time:.2f}s"
    ...
    def test_multiple_cold_start_requests(self, client):
        ...
        responses = []
        for i in range(3):
            response = client.get(f"/recommend/cold-start/{non_existent_user}?strategy=popular&limit=3")
...
class TestColdStartDataQuality:
    ...
    def test_popular_movies_availability(self, data_loader):
        ...
        popular_criteria = (movie_stats['num_ratings'] >= 50) & (movie_stats['average_rating'] >= 3.5)
    ...
    def test_genre_diversity_availability(self, data_loader):
        ...
        assert len(all_genres) >= 10, f"Should have at least 10 genres, found {len(all_genres)}"
        
        # Check for common genres
        expected_genres = ["Action", "Comedy", "Drama", "Thriller", "Romance"]
        for genre in expected_genres:
            assert genre in all_genres, f"Expected common genre '{genre}' should be available"
    ...
    def test_recent_movies_availability(self, data_loader):
        ...
        movies_with_years['year'] = movies_with_years['title'].str.extract(r'\((\d{4})\)').astype(float)
        
        # Filter recent movies (for trending strategy)
        recent_years_threshold = 2010  # Adjust based on dataset
        recent_movies = movies_with_years[movies_with_years['year'] >= recent_years_threshold]
        
        assert len(recent_movies) > 0, "Should have recent movies for trending cold start"
        assert len(recent_movies) >= 50, "Should have substantial number of recent movies"
...
class TestColdStartEdgeCases:
    ...
    def test_very_large_user_id(self, client):
        ...
        response = client.get(f"/recommend/cold-start/{very_large_user}?strategy=popular&limit=1")
    ...
    def test_extreme_limits(self, client):
        ...
        # Test minimum limit
        response = client.get(f"/recommend/cold-start/{non_existent_user}?strategy=popular&limit=1")
        assert response.status_code in [200, 500]
        
        # Test maximum allowed limit
        response = client.get(f"/recommend/cold-start/{non_existent_user}?strategy=popular&limit=50")
        assert response.status_code in [200, 500]
